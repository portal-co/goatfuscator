...
//===- CodeExtractor2.cpp - Pull code region into a new function -----------===//
//
...
static cl::opt<bool>
AggregateArgsOpt("goatf-aggregate-extracted-args", cl::Hidden,
...
CodeExtractor2::CodeExtractor2(ArrayRef<BasicBlock *> BBs, DominatorTree *DT,
...
CodeExtractor2::CodeExtractor2(DominatorTree &DT, Loop &L, bool AggregateArgs,
...
CodeExtractor2AnalysisCache::CodeExtractor2AnalysisCache(Function &F) {
  for (BasicBlock &BB : F) {
...
void CodeExtractor2AnalysisCache::findSideEffectInfoForBlock(BasicBlock &BB) {
  for (Instruction &II : BB.instructionsWithoutDebug()) {
...
bool CodeExtractor2AnalysisCache::doesBlockContainClobberOfAddr(
...
bool CodeExtractor2::isLegalToShrinkwrapLifetimeMarkers(
    const CodeExtractor2AnalysisCache &CEAC, Instruction *Addr) const {
...
BasicBlock *
CodeExtractor2::findOrCreateBlockForHoisting(BasicBlock *CommonExitBlock) {
...
CodeExtractor2::LifetimeMarkerInfo
CodeExtractor2::getLifetimeMarkers(const CodeExtractor2AnalysisCache &CEAC,
                                  Instruction *Addr,
...
void CodeExtractor2::findAllocas(const CodeExtractor2AnalysisCache &CEAC,
  ...
  // Look up allocas in the original function in CodeExtractor2AnalysisCache, as
...
bool CodeExtractor2::isEligible() const {
  if (Blocks.empty())
    return false;
...
void CodeExtractor2::findInputsOutputs(ValueSet &Inputs, ValueSet &Outputs,
...
void CodeExtractor2::severSplitPHINodesOfEntry(BasicBlock *&Header) {
...
/// outlined region.
void CodeExtractor2::severSplitPHINodesOfExits(
  ...
  for (BasicBlock *ExitBB : Exits) {
    ...
    for (PHINode &PN : ExitBB->phis()) {
      ...
      PHINode *NewPN = PHINode::Create(PN.getType(), IncomingVals.size(),
                                       PN.getName() + ".ce");
      NewPN->insertBefore(NewBB->getFirstNonPHI());
...
void CodeExtractor2::splitReturnBlocks() {
...
/// f(in0, ..., inN, out0, ..., outN)
Function *CodeExtractor2::constructFunction(const ValueSet &inputs,
...
/// necessary.
CallInst *CodeExtractor2::emitCallAndSwitchStatement(Function *newFunction,
...
void CodeExtractor2::moveCodeToFunction(Function *newFunction) {
...
void CodeExtractor2::calculateNewCallTerminatorWeights(
    BasicBlock *CodeReplacer,
...
Function *
CodeExtractor2::extractCodeRegion(const CodeExtractor2AnalysisCache &CEAC) {
...
bool CodeExtractor2::verifyAssumptionCache(const Function &OldFunc,
...
void CodeExtractor2::excludeArgFromAggregate(Value *Arg) {
