...
namespace {
struct Connect : public PassInfoMixin<Connect> {
  static char ID;
...
char Connect::ID = 0;
// static RegisterPass<Connect> X("connect", "Split & connect basic blocks & add
// garbage blocks"); Pass *createConnectPass() { return new Connect(); }
    ...
    for (BasicBlock *j : downBB) {
      ...
      if (j == destBB) {
        ...
        if (rand(g) % 2) {
          ...
          tempVal = BinaryOperator::Create(vec1Bin[rand(g) % (vec1Bin.size())],
